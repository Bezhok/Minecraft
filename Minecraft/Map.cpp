...
void Map::set_block(sf::Vector3i pos_in_chunk, Column& column, int y, block_id type)
{
	...
	if (pos_in_chunk.x >= 0 && pos_in_chunk.z >= 0 && pos_in_chunk.x < BLOCKS_IN_CHUNK && pos_in_chunk.z < BLOCKS_IN_CHUNK) {
		if (pos_in_chunk.y < BLOCKS_IN_CHUNK && pos_in_chunk.y >= 0)
			...
			temp_chunk.set_block_type(pos_in_chunk.x, Map::coord2block_coord_in_chunk(pos_in_chunk.y), pos_in_chunk.z, type);
		}
	}
...
bool Map::is_solid(int x, int y, int z)
{
	...
	return Chunk::is_block_type_solid(get_chunk_or_generate(chunk_x, chunk_y, chunk_z).get_block_type(x % BLOCKS_IN_CHUNK, y % BLOCKS_IN_CHUNK, z % BLOCKS_IN_CHUNK));
}
...
bool Map::is_opaque(int x, int y, int z)
{
	...
	return !Chunk::is_block_type_transperent(get_chunk_or_generate(chunk_x, chunk_y, chunk_z).get_block_type(x % BLOCKS_IN_CHUNK, y % BLOCKS_IN_CHUNK, z % BLOCKS_IN_CHUNK));
}
...
bool Map::is_air(int x, int y, int z)
{
	...
	return get_chunk_or_generate(chunk_x, chunk_y, chunk_z).get_block_type(x % BLOCKS_IN_CHUNK, y % BLOCKS_IN_CHUNK, z % BLOCKS_IN_CHUNK) == block_id::Air;
}
...
bool Map::is_water(int x, int y, int z)
{
	...
	return get_chunk_or_generate(chunk_x, chunk_y, chunk_z).get_block_type(x % BLOCKS_IN_CHUNK, y % BLOCKS_IN_CHUNK, z % BLOCKS_IN_CHUNK) == block_id::Water;
}
