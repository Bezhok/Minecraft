...
BlockType Chunk::get_block_type(int x, int y, int z) {
    if (m_blocks.empty()) {
...
void Chunk::generate_block_vertices(int i, int j, int k, GLfloat x, GLfloat y, GLfloat z, BlockType id) {
...
void Chunk::generate_water_vertices(int i, int j, int k, GLfloat x, GLfloat y, GLfloat z, BlockType id) {
...
void Chunk::generate_cactus_vertices(GLfloat x, GLfloat y, GLfloat z, BlockType id) {
            ...
            for (int i = 0; i < BLOCKS_IN_CHUNK; ++i)
                for (int k = 0; k < BLOCKS_IN_CHUNK; ++k)
                    ...
                    if (!is_air__in_chunk(i, j, k)) {
                        ...
                        static GLfloat pixel = 1.005 / BLOCK_RESOLUTION;
                        BlockType id = get_block_type(i, j, k);

                        if (id == BlockType::Cactus) {
...
bool Chunk::is_air__in_chunk(int x, int y, int z) {
    ...
    return get_block_type(x, y, z) == BlockType::Air;
}
...
bool Chunk::is_water__in_chunk(int x, int y, int z) {
    ...
    return get_block_type(x, y, z) == BlockType::Water;
}
...
void ChunkLayer::update(BlockType type) {
...
bool World::Chunk::is_block_type_transparent(BlockType type) {
    return type == BlockType::Air || type == BlockType::Cactus || type == BlockType::Water;
    /*|| type == block_id::Oak_leafage*/// || type == block_id::Glass; //... || type == block_id::Oak_leafage
}

bool World::Chunk::is_block_type_solid(BlockType type) {
