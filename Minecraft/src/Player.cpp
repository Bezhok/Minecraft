...
void Player::init(Map *map) {
  ...
  const Directions buffer[] = {Directions::RIGHT, Directions::LEFT, Directions::BACK, Directions::FORWARD, Directions::UP, Directions::DOWN};
...
void Player::update(double dtime) {
  ...
  auto get_oveclocking_moving = [&](Directions key) -> double {
  ...
  auto get_braking_moving = [&](Directions key) -> double {
    double dmoving = 0.f;
  ...
  auto get_moving = [&](Directions key) -> double {
    if (m_is_moving[key]) {
  ...
  // forward
  double dmoving = get_moving(Directions::FORWARD);
  m_dpos.x += -sinf(m_camera_angle.x / 180 * PI) * dmoving;
  m_dpos.z += -cosf(m_camera_angle.x / 180 * PI) * dmoving;

  // back
  dmoving = get_moving(Directions::BACK);
  m_dpos.x += +sinf(m_camera_angle.x / 180 * PI) * dmoving;
  m_dpos.z += +cosf(m_camera_angle.x / 180 * PI) * dmoving;

  // left
  dmoving = get_moving(Directions::LEFT);
  m_dpos.x += +sinf((m_camera_angle.x - 90) / 180 * PI) * dmoving;
  m_dpos.z += +cosf((m_camera_angle.x - 90) / 180 * PI) * dmoving;

  //rigth
  dmoving = get_moving(Directions::RIGHT);
  m_dpos.x += +sinf((m_camera_angle.x + 90) / 180 * PI) * dmoving;
  m_dpos.z += +cosf((m_camera_angle.x + 90) / 180 * PI) * dmoving;

  // up(jump)
  if (m_flying) {
    dmoving = get_moving(Directions::UP);

    m_dpos.y += dmoving;
    m_on_ground = false;
  } else if (m_is_moving[Directions::UP]) {
    if (m_is_in_water) {
      m_dpos.y += 3.0 * dtime;
      m_on_ground = false;
      m_direction_speed[Directions::UP] = 1.5;
    } else if (m_on_ground) {
      m_on_ground = false;
      m_direction_speed[Directions::UP] = 9.5;
    }
  }

  // lshift
  dmoving = get_moving(Directions::DOWN);
  if (m_flying) {
    m_dpos.y -= dmoving;
  }
  ...
  if (!m_flying) {
    if (!m_on_ground) {
      double s = m_direction_speed[Directions::UP];

      if (s > -80.0) {
        if (m_is_in_water) {
          if (!m_is_moving[Directions::UP]) {
            m_dpos.y -= 3.f * dtime;
          }
        } else {
          static double a = 3 * 9.8;
          s = m_direction_speed[Directions::UP] * dtime - a * dtime * dtime / 2.0;
          m_dpos.y += s;

          m_direction_speed[Directions::UP] -= a * dtime;
        }
      } else {
        m_direction_speed[Directions::UP] = -81.0;
        m_dpos.y += m_direction_speed[Directions::UP] * dtime;
      }
...
void Player::flight_on() {
  if (!m_flying) {
    ...
    m_direction_speed[Directions::UP] = 0;
  }
}
      ...
      for (int z = Converter::coord2block_coord(m_pos.z - m_size.z); z < m_pos.z + m_size.z; z++) {
        if (m_map->is_solid(x, y, z)) { //if collided with block
          ...
          if (dy < 0) {
            ...
            m_on_ground = true;
            m_direction_speed[Directions::UP] = 0.f;
          }
